...
function cancelForm(){
	...
	document.customerRegistrationForm.action="viewCustomer.htm?customerId="+customerId;
	document.customerRegistrationForm.submit();*/
	
	var customerId=document.getElementById("customerId").value;
	var url = "viewCustomer.htm";
	submitlink(url,'customerRegistrationForm');
}
...
fileReader.onload = function (event) {
  ...
  image.onload=function(){
      ...
      var ctx = context.drawImage(image,
          0,
          0,
          image.width,
          image.height,
  ...
  if(selectedImage=="profilePhoto"){
	  if(uploadFile.size <= 300000){
		  document.getElementById("newProfile").value = canvas.toDataURL('image/jpeg');
	  }else{
		  document.getElementById("newProfile").value = canvas.toDataURL('image/jpeg', 0.1);
	  }
  }else if(selectedImage=="signature"){
	  if(uploadFile.size <= 300000){
		  document.getElementById("newSignature").value = canvas.toDataURL('image/jpeg');
	  }else{
		  document.getElementById("newSignature").value = canvas.toDataURL('image/jpeg', 0.1);
	  }
  }else if(selectedImage=="idProof"){
	  if(uploadFile.size <= 300000){
		  document.getElementById("newIDproof").value = canvas.toDataURL('image/jpeg');
	  }else{
		  document.getElementById("newIDproof").value = canvas.toDataURL('image/jpeg', 0.1);
	  }
  }else if(selectedImage=="addressProof"){
	  if(uploadFile.size <= 300000){
		  document.getElementById("newAddressProof").value = canvas.toDataURL('image/jpeg');
	  }else{
		  document.getElementById("newAddressProof").value = canvas.toDataURL('image/jpeg', 0.1);
	  }
  }
...
var loadImageFile = function (imageName) {
 selectedImage = imageName;
 uploadImage = document.getElementById(imageName);
 uploadFile = document.getElementById(imageName).files[0];
